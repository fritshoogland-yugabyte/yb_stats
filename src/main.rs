...
fn main() {
    ...
    if snapshot {
        ...
        let (mut values_diff, mut countsum_diff, mut countsumrows_diff) = get_metrics_into_diff_first_snapshot(&hostname_port_vec);
        let mut statements_diff = get_statements_into_diff_first_snapshot(&hostname_port_vec);
        ...
        get_metrics_into_diff_second_snapshot(&hostname_port_vec, &mut values_diff, &mut countsum_diff, &mut countsumrows_diff, &first_snapshot_time);
        get_statements_into_diff_second_snapshot(&hostname_port_vec, &mut statements_diff, &first_snapshot_time);
